...
/**
 * This class inherits from CloseShape and draw Oval.
 */
public class EllipseComponent extends Shape {
	/**
	 * which calls the constructor of CloseShape
	 */
	 ...
	 * them into CloseShape's constructor.
	 */
	public EllipseComponent(int x1, int y1, int x2, int y2, Color color, boolean fill) {
	...
	public void draw(Graphics g) {
		...
		if (getSelected()) {
			...
			g2.draw(getRightUp());
			// g2.drawRect(getUpperLeftX(), getUpperLeftY(), 5, 5);
			// g2.drawRect(getBottomRightX(), getBottomRightY(), 5, 5);
			// g2.drawRect(getUpperLeftX(), getBottomRightY(), 5, 5);
			// g2.drawRect(getBottomRightX(), getUpperLeftY(), 5, 5);
			// g2.fill(super.getCenterpoint());
		...
		if (getFill()) // determines whether fill is true or false
			g2.fillOval(getX(), getY(), getWidth(), getHeight()); // draws a
																	// filled
																	// rectangle
		else
			g2.drawOval(getX(), getY(), getWidth(), getHeight()); // draws a
																	// regular
																	// rectangle
		// }
		g2.setStroke(s);
